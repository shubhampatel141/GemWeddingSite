export default function SectionHeading({ eyebrow, title, description, align = 'center', light = false }) {
  ...
  const textColor = light ? 'text-white' : 'text-earth-brown/72'
  const titleColor = light ? 'text-white' : 'text-dark-teal'
  const dividerColor = light ? 'bg-saffron/50' : 'bg-gold-polished/60'

  return (
    <div className={`section-heading ${alignment}`}>
      {eyebrow ? (
        <p className={`section-eyebrow ${light ? 'text-saffron' : 'text-teal-bright'}`}>
          {eyebrow}
        </p>
      ) : null}
